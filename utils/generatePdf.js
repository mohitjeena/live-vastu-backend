...
function paginateSection(sectionHtml, maxLinesPerPage = 26) {
  ...
  // If the whole section fits safely on 1 page (up to 28 lines), keep it as 1 single page!
  if (totalSectionLines <= 28) {
    return [sectionHtml];
  }
...
function paginateAiReportToPages(aiHtml) {
  ...
  // Split by <h2> sections so EVERY major title starts at the top of a page
  ...
  for (const section of rawSections) {
    const sectionPages = paginateSection(section, 26);
    for (const sp of sectionPages) {
      finalPages.push(sp);
    }
  }

  // Merge any very short trailing page into previous page if combined <= 26 lines
  ...
  for (let i = 0; i < finalPages.length; i++) {
    ...
    resultHtml += `
      <div class="vastu-page ai-report-page" style="border: 6px solid #D60000; background-color: #f7f3ef; height: 1123px; width: 100%; box-sizing: border-box; position: relative; padding: 25px 40px 0 40px; overflow: hidden;">
        ...
        <!-- Footer (Pinned to bottom: 20px, Centered with top line on every page) -->
